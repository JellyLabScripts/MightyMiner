...
public class PowderMacro extends Macro {
    ...
    public static double getRequiredYaw(double deltaX, double deltaZ) {
        double angle = Math.atan2(deltaZ, deltaX) * 180.0 / Math.PI;
        angle = normalizeAngle(angle);
        return angle;
    }

    public static double getRequiredPitch(double deltaX, double deltaY, double deltaZ) {
        double pitch = Math.atan2(Math.sqrt(deltaX * deltaX + deltaZ * deltaZ), deltaY) * 180.0 / Math.PI;
