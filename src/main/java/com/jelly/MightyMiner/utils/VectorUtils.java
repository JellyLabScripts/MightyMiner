...
public class VectorUtils {
    ...
    public static boolean anyHitOnVecNotHittable(BlockPos blockPos, Vec3 u, Vec3 v, float range) {
        Vec3 a = v.subtract(u);

        for (float i = 0; i < 11; i++) {
            Vec3 c = u.add(scaleVec(a, i / 10));
            MovingObjectPosition rayTrace = rayTraceLook(c);
    ...
    public static Pair<ArrayList<Vec3>, ArrayList<Vec3>> getAllVeryAccurateHits(final BlockPos pos) {
        final ArrayList<Vec3> hitVecs = new ArrayList<>();
        ...
        for (int x = 1; x < 9; ++x) {
            for (int y = 1; y < 9; ++y) {
                for (int z = 1; z < 9; ++z) {
                    ...
                    final MovingObjectPosition position = rayTraceLook(vec, 4.5f);
                    if (position != null && position.getBlockPos().equals((Object)pos)) {
                        hitVecs.add(position.hitVec);
                        vecs.add(vec);
                    }
                }
            }
        }
        return Pair.of(hitVecs, vecs);
    }
    ...
    public static ArrayList<BlockPos> getAllVisibleWhitelistedBlocks(ArrayList<IBlockState> whitelist, boolean blockUnderPlayer) {
        ...
        for (BlockPos blockPos: BlockPos.getAllInBox(new BlockPos(from), new BlockPos(to))) {
            if (new Vec3(blockPos).distanceTo(playerVec) < distance && whitelist.contains(BlockUtils.getBlockState(blockPos)) && isHittable(blockPos) && (blockUnderPlayer || !BlockUtils.getPlayerLoc().down().equals(blockPos))) {
